...
$(function() {
  ...
  function changeLanguage(currentLang) {
    ...
    // Hacky way to make it so once specific field is read only
    if (currentLang === 'en') {
      $('#id_department_directors-0-name').prop('readonly', false);
      $('#id_department_directors-0-name').prop('disabled', false);
    } else {
      $('#id_department_directors-0-name').prop('readonly', true);
      $('#id_department_directors-0-name').prop('disabled', true);
    }
