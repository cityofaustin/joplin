...
def test_create_event_page_with_fees():
    ...
    index = 0
    for fee in page.fees.all():
        assert fee.fee_label == expected_fees[index]["fee_label"]
        assert fee.fee == Decimal(expected_fees[index]["fee"])
        index = index + 1
...
def test_import_event_page_with_fees(remote_staging_preview_url, test_api_url, test_api_jwt_token):
    ...
    page = PageImporter(url, test_api_jwt_token).fetch_page_data().create_page()
    assert isinstance(page, EventPage)
    assert list(page.fees.all().values('fee', 'fee_label')) == [
        {'fee': Decimal('5.00'), 'fee_label': 'kids'},
        {'fee': Decimal('10.00'), 'fee_label': 'adults'},
        {'fee': Decimal('0.00'), 'fee_label': 'doggos'}
    ]
