...
class GuidePageSection(Orderable):
    page = ParentalKey(GuidePage, related_name='sections')
    heading = models.TextField(blank=True, verbose_name='Heading')
    

    panels = [
      FieldPanel('heading'),
      InlinePanel('pages', label='Select existing pages in the order you want them to display within each heading. Pages should be added only once to any single guide.'),
    ]
...
class GuidePageSectionPage(Orderable):
    section = ParentalKey(GuidePageSection, related_name='pages')
