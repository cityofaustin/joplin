...
if [ "$JANIS" == "on" ]; then
  ...
  docker-compose -f docker-compose.yml -f docker-compose.local_override.yml -f docker-compose.janis.yml run --service-ports app
else
  docker-compose -f docker-compose.yml -f docker-compose.local_override.yml run --service-ports app
fi
